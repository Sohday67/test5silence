...
// Always on — these are rare lifecycle events (attach/detach/track-count), and
// being able to see them in Console on a release build is the whole point of
// diagnosing why no audio arrives. Filter Console with: [YTSkipSilence]
...
- (BOOL)attachToPlayerItem:(AVPlayerItem *)playerItem {
    ...
    if (audioTracks.count == 0) {
        // Expected on YouTube: HLS/DASH AVURLAssets don't expose AVAssetTracks,
        // so there is nothing to hang an MTAudioProcessingTap on. Without a tap
        // there is no PCM, and without PCM there is no silence detection.
        SSLog(@"asset (%@) exposes 0 audio tracks — likely an HLS/streaming asset; "
              @"MTAudioProcessingTap cannot attach to it. Silence detection is inactive for this video.",
              [asset class]);
        [self detach];
        return NO;
    }
...
static void tapProcessCallback(MTAudioProcessingTapRef tap,
    ...
    // One-time proof-of-life: if this never prints, the tap attached but the
    // stream (HLS) is not delivering PCM to it — the silence detector can do
    // nothing without samples.
    static dispatch_once_t firstSampleOnce;
    dispatch_once(&firstSampleOnce, ^{
        NSLog(@"[YTSkipSilence][tap] FIRST audio buffer received (%ld frames) — pipeline is live",
              (long)numberFrames);
    });
