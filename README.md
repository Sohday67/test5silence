...
A **YTLite extension** that ports [Overcast](https://overcast.fm/)'s silence-skipping behavior into the YouTube iOS app. Adds a clean toggle button to the YouTube video overlay (via [PoomSmart/YTVideoOverlay](https://github.com/PoomSmart/YTVideoOverlay)). When enabled, the tweak resolves the video's progressive **AAC audio stream** from the app's own player response, decodes it in-process with `AVAssetReader` to build a **map of silence regions**, and then seeks the player past those silences (intros, outros, dead air between segments) as playback reaches them — using YouTube's own `seekToTime:`.

> **Why not tap the live audio?** YouTube plays via HLS/DASH, whose audio iOS decodes **out-of-process** in `mediaserverd`. `MTAudioProcessingTap` (the only API bridge for AVPlayer audio) explicitly excludes HLS, so the playing audio is unreachable in-process. Instead we fetch the *same* video's separate audio file (an `audio/mp4` adaptiveFormat URL, the way yt-dlp's iOS client does) and analyze **that** — it shares the video's timeline, so a silence at t=30s in the audio is a silence at t=30s in playback.
...
| Show button | On | `YTVideoOverlay-YTSkipSilence-Enabled` | Master visibility switch (managed by YTVideoOverlay). |
...
- **Requires a progressive `audio/mp4` (AAC) stream in the player response.** We
  resolve it from `playerResponse.…streamingData.adaptiveFormatsArray`. If a video
  only exposes `audio/webm` (Opus) — which `AVAssetReader` can't decode — or the
  response is HLS-manifest-only, analysis is skipped for that video. Watch Console
  (filter `[YTSkipSilence]`): `resolved audio/mp4 stream …` then `silence map ready`
  means it's working; `no audio/mp4 stream found` / `download HTTP 4xx` tells you
  why it isn't.
- **Up-front cost:** the audio file is downloaded and analyzed before skipping
  begins, so the first few seconds of a video play un-skipped, and long videos
  download more audio (the lowest-bitrate AAC track, ~0.4–1 MB/min, is chosen to
  minimise this).
- **Timeline offset content (mid-roll ads / DAI):** the analyzed audio assumes the
  player timeline matches the audio file's timeline; server-stitched ad breaks can
  shift this. Skips on such videos may be slightly off.
