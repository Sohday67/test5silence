...
This revision intentionally loads `MTAudioProcessingTap` symbols with `dlopen`/`dlsym` instead of linking `MediaToolbox.framework` directly. That avoids SDK `.tbd`/architecture link failures while still using Apple's public audio-tap API when available at runtime.
